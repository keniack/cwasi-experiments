...
def draw():
    ...
    ax: object = df.loc[:, ['W 2M', 'W 30M', 'W 100M']].boxplot(ax=axes[0],grid=True, figsize=(10, 8), positions=[3, 7, 11],
...
def draw_exec():
    ...
    ax = df.loc[:, ['O 2M','O 4M', 'O 6M','O 8M','O 10M', 'O 20M','O 40M','O 60M','O 100M']].sum().plot(
        label="Openfaas")
    ax = df.loc[:, ['C 2M','C 4M', 'C 6M','C 8M','C 10M', 'C 20M', 'C 40M','C 60M','C 100M']].sum().plot(ax=ax, color="orange",
                                                                                                              label="Cwasi")
